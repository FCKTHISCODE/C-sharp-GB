...
int findingNumberRow = Prompt("Введи cтроку числа, какого ищешь ");
int findingNumberColumn = Prompt("Введи колонку числа, какого ищешь ");
...
if (findingNumberRow > 0 && findingNumberRow <= ArrayRows && findingNumberColumn > 0
&& findingNumberColumn <= ArrayColumns)
{
    Console.WriteLine($"{GenereatedMatrix[findingNumberRow-1, findingNumberColumn-1]}");
}
else
{
    Console.WriteLine("нет такого элемеента в массиве");
}
